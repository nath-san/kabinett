...
function querySiteStats(db: Database.Database): SiteStats {
  ...
  const saneMaxYear = currentYear; // Inv.nr like 208836 gets parsed as year — cap it
  const maxYear = (db.prepare(`SELECT MAX(COALESCE(year_end, year_start)) as c FROM artworks WHERE year_start > 0 AND year_start <= ? AND COALESCE(year_end, year_start) <= ? AND ${source.sql}`).get(saneMaxYear, saneMaxYear, ...source.params) as any).c as number | null;
